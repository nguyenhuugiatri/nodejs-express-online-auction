...
module.exports = {
  ...
  getProductPriceDESC: () =>
    db.load(
      `SELECT p.id as idproduct,p.name,p.currentPrice , p.startDate, p.endDate, u.fullname ,count(b.id_product) as count from product as p LEFT JOIN user as u ON p.id_bidder = u.id LEFT JOIN biddinglist as b ON p.id = b.id_product GROUP BY p.id ORDER BY p.currentPrice DESC LIMIT 5`
    ),
  getProductEndateDESC: () =>
    db.load(
      `SELECT p.id as idproduct,p.name,p.currentPrice, p.startDate , p.endDate, u.fullname ,count(b.id_product) as count from product as p LEFT JOIN user as u ON p.id_bidder = u.id LEFT JOIN biddinglist as b ON p.id = b.id_product GROUP BY p.id ORDER BY p.endDate DESC LIMIT 5`
    ),
  getProductBiddDESC: () =>
    db.load(
      `SELECT p.id as idproduct,p.name,p.currentPrice , p.startDate , p.endDate, u.fullname ,count(b.id_product) as count from product as p LEFT JOIN user as u ON p.id_bidder = u.id LEFT JOIN biddinglist as b ON p.id = b.id_product GROUP BY p.id ORDER BY count DESC LIMIT 5`
    ),
